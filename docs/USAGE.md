...
36 套角色配色，每套 6 主色 + 深/浅变体 + 四种连续色标（seq / flow / div / cyclic）+ 配套中性色。
...
| `dist/anime-palettes-picker.pptx` | **PPT 取色板**：37 页，每页一套配色 + 原生柱状/折线图示例，用取色器直接吸色 |
...
| `dist/anime_palettes.csv` | 同样内容的纯文本表，216 行（36×6） |
| `dist/anime_palettes.json` | 完整数据，含 256 级 colormap，供二次开发 |
| `dist/ase/*.ase` | Adobe 色板（AI / PS / ID）。`_ALL-36-anime-palettes.ase` 是含 36 个分组的合集 |
...
| `docs/images/配色总览.png` | 一张图看全 36 套 |
| `docs/images/色环总览.png` | 全库 216 个色点在色轮上的分布，看还缺哪一块色相 |
...
36 套合计的相邻色差总和：平滑 4222，明度 5073，色相 5242，区分度 5685
—— 平滑约为区分度排序的 74%，36 套里有 28 套它是四者中最小的。
...
ap.wheel_all()                  # 全库 36×6 = 216 个色点的色相/彩度分布
```
...
36 套的 `flow` 全部明度单调，明度跨度 ~70 L\*，感知均匀度中位数 0.68。
...
| 等级 | 含义 | 用法 |
|---|---|---|
| **A**（18 套） | 红/绿色盲下 6 色两两 ΔE₀₀ ≥ 13 | 整套随便用，多系列图首选 |
| **B**（13 套） | 大部分可分 | 建议 4 个系列以内，或按「安全子集」取 |
...
- **AI / PS / InDesign**：`色板 → 打开色板库 → 其他库` 选 `ase/_ALL-36-anime-palettes.ase`（36 个分组），或单独导入某一套
